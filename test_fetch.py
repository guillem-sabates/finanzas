...
def chart_json(dates, closes, adj=True, currency="USD", volumes=None):
    ts = [int(dt.datetime.fromisoformat(d).replace(tzinfo=dt.timezone.utc).timestamp()) for d in dates]
...
def days(n, end="2026-07-10"):
    ...
    return [(d0 - dt.timedelta(days=i)).isoformat() for i in range(n)][::-1]
